...
module ArJdbc
  ...
  module DB2LR
    ...
    def update_lob_value?(value, column = nil)
      DB2LR.update_lob_values? && ! prepared_statements? # && value
    end
    ...
    def prefetch_primary_key?(table_name = nil)
      ...
      sql = "SELECT 1 FROM "
      sql << "SYSIBM.SYSCOLUMNS WHERE DEFAULT IN ('I','J') "
      sql << "AND TBCREATOR = '#{name.first}' " if names.size == 2
      sql << "AND TBNAME = '#{name.last}' "
      select_one(sql).nil?
    end
    ...
    def add_index(table_name, column_name, options = {})
      if table_name.to_s == ActiveRecord::Migrator.schema_migrations_table_name.to_s
